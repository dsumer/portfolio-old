...
TSDX has three different project types where you have to choose one from:

- `basic` - a plain TypeScript project (we went for this because we didn’t want to include any external libraries or were depending on React)
- `react` - choose this if you want to create a React library. TSDX takes care of all the things that have to be done to create the perfect React library out of the box
- `react-with-storybook` - same as the option before but including a Storybook setup

## Get started

TSDX makes developing your library super easy. You can just type `npm start` and your project will be started in watch mode. The entry point for your library is the `index.ts` (or `index.tsx` for React project). So when ever you will make changes in one of your corresponding files, TSDX will detect it and rebuild the library for you for super smooth development process.
...
So you know have everything adjusted to your needs, had enough time to develop your library and try it out local. You’re convinced: your library is read to be consumed by the public! So what’s next?
...
When we wanted to create a npm package for our product [snappify](https://snappify.io/) I was a bit scared because I remembered all the pain points of creating such a library some years ago. I knew of [react-hot-toast](https://react-hot-toast.com/) by [Timo Lins](https://timo.sh/) and asked him if he had some tips to get started with create a TypeScript npm package.

He pointed me to TSDX and I started to play around with it and got hooked pretty fast. The default best practices built in are just working and as the website states: you can focus on your library instead of trying to figuring out the right configurations.
