...
It seems like the best decision back then. We could move on quickly, using ExpressJS a technology we already were familiar with and didn’t have to spin up a separate service for our backend. Maybe it even was the best choice to begin with in order to let us move quickly. But for us there are some big cons to this approach which led us to the decision that we wouldn’t go this way any more in the future.
...
So we had our API running and implemented the first CRUD operations. But where should they end? 😄
...
For more details I'd recommend reading through their docs. 😄
...
Due to the custom express app, Vercel wasn't an option straight from the beginning. Back then we absolutely had no knowledge about services like AWS or DigitalOcean. I just had my own vServer running for the Browsergame I developed several years ago. That's why I thought it would be the best idea to also host TrueQ on our own vServer.
...
Because we don't want to do this by hand we chose Ansible as our operator here. With Ansible you can create playbooks which basically create the server setup for you automatically. You just tell Ansible what to do in it's own DSL written in yaml files. That means that if for whatever reason we need to setup a new server, we just need to execute the Ansible playbook there and the Server would be up and running with TrueQ.
...
For the analytics we're using the ELK stack also hosted on those vServers. We're collecting logs via filebeat and metrics with metricbeat. Additionally we're having the Kibana APM in place to see real time insights into our Node.js backend application.

Also this was a very interesting step, but again too much hassle to maintain this on our own.

## GitLab CI/CD

The process of building a docker image and deploying it with Ansible is all encapsulated in mostly automated steps in our GitLab pipelines.

# Test Setup

We talked about the stack we developed our application with and talked about how we're deploying it. Now comes a topic which 

TDD was an acronym we heard pretty often in our career before, but never came into an environment which .

# What would you do differently nowadays?

BlitzJS, AWS, Vercel, DigitalOcean, Supabase, Firebase

We're very glad that the first MVP of our new product [snappify](https://snappify.io) comes with barely any backend logic and is completely hosted on Vercel, but soon we need a solution and I am going to inform you in the future how our updated tech stack is going look like. ✌️